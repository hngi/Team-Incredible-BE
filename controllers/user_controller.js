...
module.exports = {
    create: async (req, res) =>{
        ...
        const { first_name, last_name,email,phone,password, cpassword} = req.body;
    ...
    login: async (req, res) =>{
        //validate the user first
          ...
          //validate the user
          try {
             const found= users.some(users=>users.email === email && users.password === password);
            if(found){
              //  res.json(users.filter(users=>users.email===email));
                res.status(200).json({message:"Success",Token: uu.v4(), found:found})
